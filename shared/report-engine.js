...
} from './maya-calendar-core.js';
import { resolveOrderReportVariant } from './report-versions.js';
...
export function calculateMayanSignature({ day, month, year }) {
  ...
  const tone = GALACTIC_TONES[position.toneNumber - 1];
  const sign = DAY_SIGNS[position.daySignIndex];
  return {
    sign,
    tone,
    signature: `${tone.number} ${sign.name}`,
    ...position,
  };
}
...
export function renderReportHtml(input, options = {}) {
  ...
  const reportOffer = resolveOrderReportVariant(order, options);
  const isTransparentReport = reportOffer.key === 'transparent_v3';
  ...
  const coverNumberLabel = isTransparentReport ? 'Tzolk’in number' : 'Galactic Tone';
  const mapNumberLabel = isTransparentReport ? 'Your Tzolk’in Number' : 'Your Tone';
  const insideNumberLabel = isTransparentReport ? 'Tzolk’in number · modern Dreamspell name' : 'Galactic Tone';
  const numberSectionLabel = isTransparentReport ? '02 · Your Tzolk’in Number' : '02 · Your Galactic Tone';
  const numberHeading = isTransparentReport
    ? `Number ${tone.number} · Modern Dreamspell name: ${escapeHtml(tone.name)}`
    : `Tone ${tone.number} · ${escapeHtml(tone.name)}`;
  const numberIntroduction = isTransparentReport
    ? `The number is part of the traditional 1–13 Tzolk’in count. The name ${escapeHtml(tone.name)} and the following reflective keywords come from the modern Dreamspell interpretive layer: ${escapeHtml(tone.meaning)}.`
    : `The tone describes how your sign’s energy tends to move: through ${escapeHtml(tone.meaning)}.`;
  const numberRhythmCopy = isTransparentReport
    ? 'Think of the Nawal as the quality of energy and the number as its working rhythm in this modern reflective model.'
    : 'Think of the Nawal as the quality of energy and the tone as its working rhythm.';
  const numberPracticeHeading = isTransparentReport ? 'Three ways to work with this number' : 'Three ways to work with this tone';
  const numberQuoteLabel = isTransparentReport ? 'Number' : 'Tone';
  const methodologyCopy = isTransparentReport
    ? `<p>The date calculation uses the GMT correlation constant 584283 to derive a Tzolk’in named day and number. Calculation version: ${escapeHtml(MAYA_CALCULATION_VERSION)}.</p><p>The name ${escapeHtml(tone.name)} and its process keywords are a modern Dreamspell interpretation, not an ancient Maya term. The longer personality, relationship, and work passages are modern English-language reflective synthesis. This report does not claim lineage authority or represent a complete account of living Maya traditions.</p><p>Read the full methodology and sources at https://mayanastrologycalculator.com/methodology.</p>`
    : `<p>The date calculation uses the widely used GMT correlation constant to derive a Tzolk’in day sign and tone. The interpretive synthesis is modern, English-language reflective content. It does not claim lineage authority and should not be treated as a complete account of living Maya traditions.</p>`;
  ...
  ${page('', `<span class="section-label">${numberSectionLabel}</span><h2>${numberHeading}</h2><p class="lede">${numberIntroduction}</p><p>${numberRhythmCopy} When you respect that rhythm, effort becomes more coherent. When you force another person’s pace, the same strengths may feel scattered or heavy.</p><div class="card"><h3>${numberPracticeHeading}</h3><ol class="steps">${tone.practices.map((item) => `<li>${escapeHtml(item)}.</li>`).join('')}</ol></div><h3>When the rhythm is strained</h3><p>Pause before adding more effort. Ask whether the moment needs action, definition, cooperation, release, or simply presence. The answer may be smaller and more practical than the pressure suggests.</p><p class="quote">${numberQuoteLabel} ${tone.number} asks: “What rhythm lets ${escapeHtml(sign.name)} become useful today?”</p>`)}
  ...
  ${page('', `<span class="section-label">Closing reflection</span><h2>Your Report Belongs to You</h2><p class="lede">Keep the language that helps you see more clearly. Release any interpretation that narrows your freedom or contradicts lived experience.</p><h3>Reflection prompts</h3><ul><li>Which description felt immediately recognizable?</li><li>Which growth edge deserves curiosity rather than judgment?</li><li>Where could your signature become service without becoming self-sacrifice?</li><li>What is one practice you will repeat for the next month?</li></ul><div class="divider"></div><h3>Methodology and cultural scope</h3>${methodologyCopy}<p>This report is educational and reflective. It is not scientific, medical, legal, financial, or psychological advice.</p><div class="promise"><h3>Our delivery promise</h3><p>If your paid report cannot be delivered or the file is unusable, contact support@mayanastrologycalculator.com within 7 days so we can replace it or refund the purchase.</p></div><p class="small">Prepared by Mayan Astrology Calculator · ${escapeHtml(REPORT_PRODUCT.name)} v${reportOffer.reportVersion}</p>`)}
  </main></body></html>`;
}
